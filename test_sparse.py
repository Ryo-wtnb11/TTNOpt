...
if __name__ == "__main__":
    ...
    sparse_z = BlockSparseTensor.fromdense([Index(zcharge, flow=False), Index(zcharge, flow=True)], pauli_z)
    sparse_p = BlockSparseTensor.fromdense([Index(pcharge, flow=False), Index(zcharge, flow=True)], pauli_p)
    sparse_m = BlockSparseTensor.fromdense([Index(mcharge, flow=False), Index(zcharge, flow=True)], pauli_m)
    sparse_x = BlockSparseTensor.fromdense([Index(xycharge, flow=False), Index(zcharge, flow=True)], pauli_x)
    sparse_y = BlockSparseTensor.fromdense([Index(xycharge, flow=False), Index(zcharge, flow=True)], pauli_y)

    u1 = U1Charge([0, 1])
    u2 = U1Charge([0, 1])
    u_in = U1Charge([0, 1, 1, 2])
    isometry_sparse = BlockSparseTensor.random([Index(u1, flow=True), Index(u2, flow=True), Index(u_in, flow=False)])
    ...
    ket[0] ^ spin_z[1]
    bra[1] ^ ket[1]
    Sz_dense = tn.contractors.auto([bra, spin_z, ket], output_edge_order=[bra[2], ket[2]])
    ...
    ket[0] ^ spin_p[1]
    bra[1] ^ ket[1]
    Sp_dense = tn.contractors.auto([bra, spin_p, ket], output_edge_order=[bra[2], ket[2]])
    print(Sp_dense.tensor)

    print("S-")
    bra = tn.Node(isometry)
    ket = bra.copy(conjugate=True)
    spin_m = tn.Node(pauli_m)

    bra[0] ^ spin_m[0]
    ket[0] ^ spin_m[1]
    bra[1] ^ ket[1]
    Sm_dense = tn.contractors.auto([bra, spin_m, ket], output_edge_order=[bra[2], ket[2]])
    print(Sm_dense.tensor)
    ...
    isometry_sparse_ket = BlockSparseTensor.fromdense([Index(u1, flow=True), Index(u2, flow=True), Index(u_in_p, flow=False)], isometry_sparse.todense())
    ...
    u1 = U1Charge([-1, 0])
    u2 = U1Charge([0, 1])
    u_in_m = U1Charge(U1Charge.fuse([-1, 0], [0,1]))
    isometry_sparse_bra = BlockSparseTensor.fromdense([Index(u1, flow=True), Index(u2, flow=True), Index(u_in_m, flow=False)], isometry_sparse.todense())
    bra = tn.Node(isometry_sparse_bra, backend="symmetric")
    ket = tn.Node(isometry_sparse, backend="symmetric").copy(conjugate=True)
    spin_m = tn.Node(sparse_m, backend="symmetric")
    bra[0] ^ spin_m[0]
    ket[0] ^ spin_m[1]
    bra[1] ^ ket[1]
    Sm = tn.contractors.auto([bra, spin_m, ket], output_edge_order=[bra[2], ket[2]])
    print(Sm.tensor.todense())
