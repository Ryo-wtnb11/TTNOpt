...
def hamiltonian(config: DotMap):
    ...
    if interaction_indices.max() >= config.N:
        raise ValueError(
            f"Interaction indices exceed the allowed range. All indices must be less than N-1 (N={config.system.N})."
        )
    if interaction_indices.shape[0] != interaction_coefs.shape[0]:
        raise ValueError(
            "Number of rows in Jij_file must match number of rows in ij_file"
        )
    ...
    if config.MF_X.active == 1:
        if config.MF_X.uniform == 1:
            ...
            if isinstance(config.MF_X.h, DotMap):
                raise ValueError(
                    "Please input h value in magnetic_field if uniform is 1"
                )
            ...
            if isinstance(config.MF_X.hi_file, DotMap):
                raise ValueError(
                    "Please input hi_file in magnetic_field if uniform is 0"
                )
            magnetic_field_X_csv = pd.read_csv(
                config.MF_X.hi_file, delimiter=",", header=None
            )
    ...
    if config.MF_Y.active == 1:
        if config.MF_Y.uniform == 1:
            ...
            magnetic_field_Y_csv = pd.read_csv(
                config.MF_Y.hi_file, delimiter=",", header=None
            )
    ...
    if config.MF_Z.active == 1:
        if config.MF_Z.uniform == 1:
            ...
            magnetic_field_Z_csv = pd.read_csv(
                config.MF_Z.hi_file, delimiter=",", header=None
            )
    ...
    if config.SIA.active == 1:
        if config.SIA.uniform == 1:
            ...
            if isinstance(config.SIA.D, DotMap):
                raise ValueError(
                    "Please input D value in ion_anisotropy if uniform is 1"
                )
            ...
            if isinstance(config.SIA.Di_file, DotMap):
                raise ValueError(
                    "Please input Di_file in ion_anisotropy if uniform is 0"
                )
            ion_anisotropy_csv = pd.read_csv(
                config.SIA.Di_file, delimiter=",", header=None
            )
    ...
    if config.DM_X.active == 1:
        ...
        dzyaloshinskii_moriya_X_csv = pd.read_csv(
            config.DM_X.DM_file, delimiter=",", header=None
        )
    ...
    if config.DM_Y.active == 1:
        ...
        dzyaloshinskii_moriya_Y_csv = pd.read_csv(
            config.DM_Y.DM_file, delimiter=",", header=None
        )
    ...
    if config.DM_Z.active == 1:
        ...
        dzyaloshinskii_moriya_Z_csv = pd.read_csv(
            config.DM_Z.DM_file, delimiter=",", header=None
        )
